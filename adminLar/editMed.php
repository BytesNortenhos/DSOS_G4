...
<body class="bg-primary flex justify-top">
    <div class="w-screen flex flex-col items-center justify-center h-screen">
        <h1 class="text-secondary text-6xl font-bold">EDITAR MEDICAMENTO</h1>
        <a class="text-secondary text-xl">No seguinte formulário poderá ediitar o medicamento selecionado!</a>
        <form class="flex flex-col justify-center items-center content-center mt-12 w-full" action="editaMed.php"
            method="POST" onsubmit="editMed(); return false;">
            <input
                class="h-10 w-96 rounded-2xl bg-primary border-solid border-secondary border-2 px-2 pl-2 text-secondary"
                type="text" id="nomeMed" value="<?php echo $_GET['nomeMed'] ?>">
            <input
                class="h-10 w-96 mt-5 rounded-2xl bg-primary border-solid border-secondary border-2 px-2 pl-2 text-secondary"
                type="text" id="marcaMed" value="<?php echo $_GET['marca'] ?>">
                <input
                class="h-10 w-96 mt-5 rounded-2xl bg-primary border-solid border-secondary border-2 px-2 pl-2 text-secondary"
                type="text" id="princAtivo" value="<?php echo $_GET['princAtivo'] ?>">
                <input
                class="h-10 w-96 mt-5 rounded-2xl bg-primary border-solid border-secondary border-2 px-2 pl-2 text-secondary"
                type="text" id="doseMed" value="<?php echo $_GET['dose'] ?>">
                <input
                class="h-10 w-96 mt-5 rounded-2xl bg-primary border-solid border-secondary border-2 px-2 pl-2 text-secondary"
                type="text" id="tomaMed" value="<?php echo $_GET['toma'] ?>">
            <button
